 ...
 * @editor     : Ethan<ethanshan@foxmail.com>
 * @date       : 2015-12-24
 */
class parserDom {
    /**
    ...
    public function firstChild($node = NULL){
        ...
        if($node->firstChild){
            if($node->firstChild->nodeType === XML_TEXT_NODE){
                return $this->nextSibling($node->firstChild);
            }else{
                return new self($node->firstChild);
            }
        }else{
            return NULL;
        }
    ...
    public function nextSibling($node = NULL){
        ...
        if($node->nextSibling){
            if($node->nextSibling->nodeType === XML_TEXT_NODE){
                return $this->nextSibling($node->nextSibling);
            }else{
                return new self($node->nextSibling);
            }
        }else{
            return NULL;
        }
